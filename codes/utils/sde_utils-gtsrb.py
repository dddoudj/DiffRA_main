...
classifier = GtsrbCNN(n_class=43)
classifier.load_state_dict(
    torch.load(f'/home/liuwenjie/refusion/codes/utils/torch_nets_weight/model_gtsrb.pth'))
...
class IRSDE(SDE):
    ...
    '''def reverse_sde(self, xt, T=-1, save_states=False, save_dir='sde_state', **kwargs):
        ...
        return x'''
    def reverse_sde(self, xt, x0, T=-1, save_states=False, save_dir='sde_state', **kwargs):  # grstb
        T = self.T if T < 0 else T
        x = xt.clone()
        ...
        pred1 = classifier(x)
        x_rpre1 = classifier(x0)
        pred_indexadv1 = torch.argmax(pred1, 1).cpu().detach().numpy()
        pred_indexr1 = torch.argmax(x_rpre1, 1).cpu().detach().numpy()

        if pred_indexadv1 != pred_indexr1:
            success1 = 1
        else:
            success1 = 0

        print("the GT current predict class name: %s" % pred_indexr1[0])
        print("the advsde current predict class name: %s" % pred_indexadv1[0])
