...
classifier = torchvision.models.inception_v3(pretrained=True).eval()
...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            '''#print("class_gt", classifier(GT).argmax(1))
            x_pre = classifier(GT).argmax(1)
            loss_fn = torch.nn.CrossEntropyLoss(reduction="sum")
            GT_adv = GT.clone()
            input_range = (-1, 1)
            alpha = 0.1 / 255.0 * (input_range[1] - input_range[0])
            iter = 8
            for pgd_iter_id in range(iter):
                with torch.enable_grad():
                    GT_adv = GT_adv + torch.randn_like(GT_adv) * 0.01
                    GT_adv.requires_grad_()
                    loss0 = loss_fn(classifier(GT_adv), x_pre)
                    loss0.backward()
                    grad_sign0 = GT_adv.grad.data.sign()
                    delta = grad_sign0 * alpha
                    GT_adv = (GT_adv + delta).detach()
            #print("class_gtadv", classifier(GT_adv).argmax(1))'''


            model.feed_datatest(states, LQ, GT) # xt, mu, x0
            #model.feed_data(states, LQ, GT, GT_adv)
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    # valid Predictor
                    model.feed_datatest(noisy_state, LQ, GT)
                    model.test(sde)
