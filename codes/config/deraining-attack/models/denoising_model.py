...
class DenoisingModel(BaseModel):
    ...
    #def feed_data(self, state, LQ, GT=None, img2=None):
        ...
        '''if img2 is not None:
            self.state_2 = img2.to(self.device)'''
    ...
    def test(self, sde=None, save_states=False):
        ...
        with torch.no_grad():
            ...
            self.output, self.success0 = sde.reverse_sde(
                self.state, self.state_0, save_states=save_states)

        self.model.train()

    def attacksuccess(self):
        num0 = self.success0
        return num0

    '''def test(self, sde=None, save_states=False):     #gtsrb
        sde.set_mu(self.condition)
        self.model.eval()
        with torch.no_grad():
            self.output, self.success = sde.reverse_sde(self.state, self.state_0, save_states=save_states)
        self.model.train()
        
    def attacksuccessgtsrb(self):
        num1 = self.success
        return num1'''
